<script>
    ...
    import { currentStep, customerEmail, customerName, customerPassword, emailIsError, nameIsError, passwordIsError } from "./sharedState";
    import {customerLastnames, customerMatricula, lastnamesIsError, matriculaIsError } from "./sharedState";
    import { customerGroup, customerCarreer, customerAlergy, groupIsError, carreerIsError, alergyIsError } from "./sharedState";
    ...
    <main>
        ...
        <footer>
            <button class="previousButton" on:click={()=>{$currentStep--}} class:show="{$currentStep > 1 && $currentStep < 6}"> Regresar </button>
            {#if $currentStep < 5}
                <button class="nextButton" on:click={nextStep}> Siguiente </button>
            {:else if $currentStep === 5}
                <button class="nextButton confirm" on:click={()=>{$currentStep++}}> Confirmar </button>
            {/if}
        </footer>
    </main>
